...
- 凭证**可以读取**（2026-09-25 起；管理员凭据 2026-09-26 用户再次明确）：API key、
  OAuth 访问 token、刷新 token、client secret、加密凭证文件与**管理员凭据**（管理后台
  登录邮箱/用户名与口令），都可以由代理从本机配置、环境变量或凭证文件读出，用于执行
  `yarn microfeed` 等本机操作与真实浏览器 E2E 验证（如 `scripts/e2e/` 下的脚本）。
  读取到的值只许留在进程内：**绝不**打印到对话、**绝不**写入任何文件或日志（含
  `.microfeed/` 下的日志、脚本与测试夹具）、**绝不**复制到别处、**绝不**提交。
  凭证始终属于操作者，代理只是使用者。
