...
import {jsonResponse, publicLocalizedError, publicLocalizedTextError} from "@/server/http";
import {
...
export const searchApiItems: APIRoute = async ({locals, request}) => {
  if (!locals.feedDb) {
    return publicLocalizedTextError(request, "errors.general.feedContextUnavailable", 500);
  }
  const parsed = apiSearchQuerySchema.safeParse(
    Object.fromEntries(new URL(request.url).searchParams),
  );
  if (!parsed.success) {
    return publicLocalizedError(request, "errors.api.invalidSearchQuery", 400);
  }
  try {
...
export const listApiPages: APIRoute = async ({locals, request}) => {
  ...
  if (
    ...
    return publicLocalizedError(request, "errors.api.invalidPageListQuery", 400);
  }
  try {
...
export const getApiPage: APIRoute = async ({locals, params, request}) => {
  ...
  return page
    ? jsonResponse(page)
    : publicLocalizedError(request, "errors.page.notFound", 404);
};
...
export const updateApiPage: APIRoute = async ({locals, params, request}) => {
  ...
  try {
    ...
    if (!page) return publicLocalizedError(request, "errors.page.notFound", 404);
...
export const deleteApiPage: APIRoute = async ({locals, params, request}) => {
  ...
  try {
    ...
    if (!await deletePage(
      ...
      return publicLocalizedError(request, "errors.page.notFound", 404);
    }
    return jsonResponse({});
  } catch (error) {
    const response = pageServiceError(error);
...
export const createApiSiteFile: APIRoute = async ({locals, request}) => {
  ...
  if (!parsed.success || !parsed.data.filename) {
    return publicLocalizedError(request, "errors.siteFile.invalid", 400);
  }
  try {
...
export const previewApiSiteFile: APIRoute = async ({locals, request}) => {
  ...
  try {
    ...
    return preview
      ...
      : publicLocalizedError(request, "errors.siteFile.notFound", 404);
  } catch (error) {
    const response = siteFileServiceError(error);
    if (response) return response;
    throw error;
...
export const updateApiSiteFile: APIRoute = async ({
  ...
  try {
    ...
    if (!siteFile) {
      return publicLocalizedError(request, "errors.siteFile.notFound", 404);
    }
...
export const createApiItem: APIRoute = async ({locals, request}) => {
  ...
  if (!parsed.success) {
    return publicLocalizedError(request, "errors.item.invalid", 400);
  }
  ...
  if (!idempotencyKey.success) {
    return publicLocalizedError(request, "errors.api.invalidIdempotencyKey", 400);
  }
  if (!locals.feedDb) {
    return publicLocalizedTextError(request, "errors.general.feedContextUnavailable", 500);
  }
...
export const validateApiItem: APIRoute = async ({request}) => {
  ...
  return parsed.success
    ...
    : publicLocalizedError(request, "errors.item.invalid", 400);
};
...
export const deleteApiItem: APIRoute = async ({locals, params, request}) => {
  ...
  if (!itemId) {
    return publicLocalizedError(request, "errors.item.invalidId", 400);
  }
  if (!locals.feedCrud || !locals.feedDb) {
    return publicLocalizedTextError(request, "errors.general.feedContextUnavailable", 500);
  }
...
export const updateApiPrimaryChannel: APIRoute = async ({
  ...
  if (params.channelId !== "primary") {
    return publicLocalizedError(request, "errors.channel.invalidId", 400);
  }
  if (!locals.feedCrud) {
    return publicLocalizedTextError(request, "errors.general.feedContextUnavailable", 500);
  }
  ...
  if (!parsed.success) {
    return publicLocalizedError(request, "errors.channel.invalid", 400);
  }
...
export const prepareApiMediaUpload: APIRoute = async ({locals, request}) => {
  ...
  if (!parsed.success) {
    return publicLocalizedError(request, "errors.api.invalidMediaUpload", 400);
  }
  ...
  if (category !== "image" && !itemId) {
    return publicLocalizedError(request, "errors.api.mediaItemIdRequired", 400);
  }
  ...
  try {
    signed = await createSignedUpload(request, env, {
      key,
      size: input.size,
      type: input.type,
    });
    ...
    if (error instanceof AppError) {
      return publicLocalizedError(request, error.i18nKey, error.status, error.params);
    }
    throw error;
  }
