...
export function AdminSideQuickLinks({AdditionalLinksDiv}: any) {
  ...
  return (<div className="mt-4 rounded-[14px] border bg-card p-5 text-card-foreground shadow-xs">
    <div className="mb-2 text-sm font-semibold text-foreground">{t("shared.publicAccess")}</div>
    <div className="grid grid-cols-1 gap-2">
      {AdditionalLinksDiv}
      <div className="flex flex-wrap gap-y-2">
        <SideQuickLink url={PUBLIC_URLS.webFeed()} text={t("shared.feedWeb")}/>
        <SideQuickLink url={PUBLIC_URLS.jsonFeed()} text={t("shared.feedJson")}/>
        <SideQuickLink url={PUBLIC_URLS.rssFeed()} text={t("shared.feedRss")}/>
      </div>
    </div>
  </div>);
}
